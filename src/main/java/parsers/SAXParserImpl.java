...
public class SAXParserImpl extends DefaultHandler implements Parser {
    ...
    public void parse(String fileName) throws ParserConfigurationException, SAXException, IOException {
        ...
        AgeFilter ageFilter = new AgeFilter(30, 50);
        InsuranceFilter insuranceFilter = new InsuranceFilter(true);
        Filter filter = Filter.builder()
                .addFilter(ageFilter)
                .addFilter(insuranceFilter)
                .build();
        List<Patient> filteredPatients = filter.doChain(patients);
        filteredPatients.forEach(PrintEntityUtils::printPatientInfo);
    }
